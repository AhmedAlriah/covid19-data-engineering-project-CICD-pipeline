...
function Update-TriggerTemplate {
    ...
    foreach ($parameterMatch in $parameterMatches) {
        ...
        if (($null -ne $templateParameters.$($parameterName)) -and ($PreDeployment -or $parameterName.EndsWith('runtimeState'))) {
            if ($null -ne $templateParameters.$($parameterName).value) { $parameterType = $templateParameters.$($parameterName).value.GetType().Name } else { $parameterType = $null }
